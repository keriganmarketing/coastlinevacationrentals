<?php
	...
	/* Override $from_email in wp_mail */

	function override_mail_from( $email ) {

		$domain = defined( 'DOMAIN_CURRENT_SITE' ) ? constant( 'DOMAIN_CURRENT_SITE' ) : false;

		if ( ! $domain ) {
			$db_url_parts = parse_url( site_url() );
			$domain       = str_replace( 'www.', '', $db_url_parts['host'] );
		}
	...
	function enqueue_footer() {
		wp_enqueue_script( 'unveil', get_relative(plugins_url( '/js/jquery.unveil.js', __FILE__)), array('jquery-min'), null ,true );
		wp_enqueue_script( 'cookie', get_relative(plugins_url( '/js/jquery.cookie.js', __FILE__)), array('jquery-min'), null ,true );
		wp_enqueue_script( 'scripts', get_relative(plugins_url( '/js/scripts.js', __FILE__)), array('unveil', 'cookie', 'flexslider'), null ,true );
	}
	add_action('wp_enqueue_scripts', 'enqueue_footer');

	function enqueue_and_register_admin_scritps() {

		wp_register_script( 'kigo-plugin-admin-js', get_relative( plugins_url( '/js/admin.js', __FILE__) ), array( 'jquery-migrate', 'jquery-ui-button', 'jquery-ui-tabs', 'jquery-ui-dialog' ), false, false );
...
function myplugin_meta_box_callback( $metaId ) {
	wp_nonce_field( 'insta_seo_metabox ', 'myplugin_meta_box_nonce' );
	 ...
	 $newUrlray = explode('http://localhost', $url);
...
if (isset($_GET['page']) && $_GET['page'] == 'site_settings_initial') {
    add_action('login_enqueue_scripts', 'site_logo_scripts');
    add_action('login_enqueue_scripts', 'site_logo_styles');
}
...
function my_page_template_redirect()
{
	...
	if(isset($pages[$slug]) && !is_page($slug)) {
