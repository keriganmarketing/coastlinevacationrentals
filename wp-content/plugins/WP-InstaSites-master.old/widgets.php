...
class BAPI_HP_Logo extends WP_Widget {
	...
	public function widget( $args, $instance ) {
	    ...
	    ?>
		<div class="bapi-logo"><a href="<?= $url ?>" ><img src="<?php echo esc_url( get_option('Site_Logo') ); ?>" alt="" /></a></div>     
	   <?php
	   }
	   else{?>
		...
		<div id="bapi-search">
			<div class="property-search-block">
				<form name="property-search" method="get" action="<?php echo $action; ?>">
						...
						<div class="category-block">
							<select name="location" class="span12 property-search-input quicksearch sessionlocation" data-field="location">
								...
								<?php foreach($solutionConfig['location']['hierarchicalValues'] as $value) { ?>
									<option value="<?php echo $value['Data']; ?>" class="level1"><?php echo $value['Label']; ?>&nbsp;(<?php echo $value['PropertyCount']; ?>)</option>
									<?php foreach($value['Children'] as $child1) { ?>
									<option value="<?php echo $child1['Data']; ?>" class="level2">&nbsp;&nbsp;<?php echo $child1['Label']; ?>&nbsp;(<?php echo $child1['PropertyCount']; ?>)</option>
										<?php foreach($child1 as $child2) { ?>
											<option value="<?php echo $child2['Data']; ?>" class="level3">&nbsp;&nbsp;--&nbsp;<?php echo $child2['Label']; ?>&nbsp;(<?php echo $child2['PropertyCount']; ?>)</option>
											<?php foreach($child2 as $child3) { ?>
												<option value="<?php echo $child3['Data']; ?>" class="level4">&nbsp;&nbsp;&nbsp;--&nbsp;<?php echo $child3['Label']; ?>&nbsp;(<?php echo $child3['PropertyCount']; ?>)</option>
											<?php } ?>
										<?php } ?>
									<?php } ?>
								<?php } ?>
					...
					?>
					<script>var headlines = <?php echo json_encode($titles); ?>;</script>
					<div class="category-block">
...
class BAPI_DetailOverview_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		<?php
			...
			$secure_url = !defined('KIGO_DEBUG') || false == KIGO_DEBUG && isset($soldata['SecureURL']) ? '//'.$soldata['SecureURL'] : '';
