...
context.createRateBlockWidget = function (target, options){
	...
	BAPI.datamanager.get(BAPI.entities.property, cur.ID, function(p) {
		...
		//we set the calendars from session
		setCalendarsFromSession(data.session.searchparams,'#rateblockcheckin','#rateblockcheckout');
...
context.createSearchWidget = function (targetid, options, doSearchCallback) {
	...
	//set the calendars from the session
	setCalendarsFromSession(BAPI.session.searchparams,'#searchcheckin','#searchcheckout');
    ...
    function setCalendarsFromSession(session,checkinSelector,checkoutSelector){
        if(!BAPI.isempty(session) && !BAPI.isempty(session.checkin) && $(checkinSelector).length > 0){
            if($(checkinSelector).data('pickadate')){
                setCalendars();
            }else{
                $(window).load(setCalendars);
            }
        }
        ...
        function setCalendars(){
            ...
            if(session.checkout !== null && $(checkoutSelector).length > 0){
                $(checkoutSelector).data('pickadate').set('select',session.checkout,{format: BAPI.defaultOptions.dateFormatBAPI.toLowerCase() } );
            }
        }
    }
			...
			onSet: function(event) {
				...
				if (theCheckOutElement!==null && theCheckOutElement.length>0) {
					...
					if ( event.select ) { 
						...
						var theChkOutDate = moment(event.select, BAPI.defaultOptions.dateFormat.toUpperCase()).add(addDay, 'days').format(BAPI.defaultOptions.dateFormat.toUpperCase());
...
function bookingHelper_FullLoad(targetid, options, propid) {
	...
	var propoptions = { avail: 1, seo: 1, loadconfig: 1 };
	$.extend(propoptions, BAPI.session.searchparams);
...
function PaymentHelper_FullLoad(targetid, options, bid) {
    var propoptions = { avail: 1, seo: 1 ,"renterEmail": options.renterEmail }
